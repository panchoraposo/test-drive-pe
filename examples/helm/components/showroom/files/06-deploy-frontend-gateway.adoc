...
IMPORTANT: The scaffolded frontend does **not** implement OIDC login (no redirect to Keycloak). It uses API Keys directly via the `X-API-Key` header. For an interactive OIDC flow with Keycloak, see the pre-deployed `neuralbank-stack` in xref:09-explore-connectivity-link-oidc.adoc[the OIDC module].
